...
struct ReceiptParserTests {
    ...
    // MARK: - New regression tests for regex fix

    @Test("standard XX.XX prices: 2 items, correct subtotal")
    func standardDecimalPrices() {
        let lines = [
            "Burger 12.50",
            "Fries 4.00",
            "Total 16.50",
        ...
        let receipt = ReceiptParser.parseText(lines)
        #expect(receipt.items.count == 2)
        #expect(receipt.subtotal == 16.50)
    }

    @Test("dollar-sign prices: $18.5 and $12 parsed, tax extracted")
    func dollarSignPrices() {
        let lines = [
            "Pasta $18.5",
            "Wine $12",
            "Tax $3.00",
        ...
        let receipt = ReceiptParser.parseText(lines)
        #expect(receipt.items.count == 2)
        #expect(receipt.tax == 3.00)
        #expect(receipt.items[0].price == 18.5)
        #expect(receipt.items[1].price == 12.0)
    }

    @Test("mixed dollar-sign and plain decimal formats")
    func mixedFormat() {
        let lines = [
            "Club Sandwich 14.00",
            "Diet Coke $3.5",
        ...
        let receipt = ReceiptParser.parseText(lines)
        #expect(receipt.items.count == 2)
        #expect(receipt.subtotal == 17.50)
        #expect(receipt.tax == 1.58)
        #expect(receipt.tip == 3.00)
        #expect(receipt.total == 22.08)
    }

    @Test("item name has price and dollar sign stripped, no trailing whitespace")
    func itemNameCleanup() {
        let lines = [
            "Club Sandwich $14.00",
            "Total $14.00",
        ]
        let receipt = ReceiptParser.parseText(lines)
        #expect(receipt.items.count == 1)
        let name = receipt.items[0].name
        #expect(!name.hasSuffix(" "))
        #expect(!name.contains("$"))
        #expect(!name.contains("14.00"))
        #expect(name == "Club Sandwich")
    }
}
